...
class index extends Controller
{
        ...
        foreach ($uniqueTitles as $data) {
            ...
            if (isset(Auth::user()->role)) {
                if (Auth::user()->role == 'admin') {
                    ...
                    <a href="' . route("admin.faqsUpdate.title", ['id' => '' . $data->id . '',
                    'sub_path_admin'=>$this->sub_path_admin(),]) . '"  class="btn btn-open hover-bg-green-700 text-white rounded-md" style="margin-left: 10px; margin-bottom: 10px;background-color: black;">
                    Modifier</a>
            ...
            $faqTitle = $this->faq->getCollectionFaq('title_id', $data->id);
            ...
            foreach ($faqTitle as $faq) {
                ...
                if (isset(Auth::user()->role)) {
                    if (Auth::user()->role == 'admin') {
                        ...
                        $html .= '<a href="' . route("admin.faqsUpdate", ['id' => '' . $faq->id . '',
                        'sub_path_admin'=>$this->sub_path_admin(),]) . '"  class="btn hover:bg-green-700 text-white rounded-md" style="margin-left: 10px; margin-bottom: 10px;background-color: black;">
                        Modifier</a>
    ...
    {
        $this->faq->DeleteFaq('id', $id);
        return redirect()->back();
    }

    public function deleteforfaqtitle($id)
    {
        $this->faq->DeleteFaq('title_id', $id);
        $this->FaqT->DeleteFaq_title('id', $id);
        return redirect()->back();
    }
}
